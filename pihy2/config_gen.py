...
def redact_secrets(text: str) -> str:
    """把渲染后配置里的敏感值替换成 ``******``（仅用于预览/打印，避免截图或
    ``pihy2 config | tee`` 把凭据泄露到日志/滚屏）。

    按「键名行」整体替换，与具体取值无关——绝不会误伤普通字段，也不会因密码恰好是
    某段子串而错误替换或破坏 YAML（朴素 str.replace 会有这种风险）。
    """
    return _SECRET_LINE_RE.sub(r'\1"******"', text)
