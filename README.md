...
- **WebUI 面板**：部署后自带中文管理面板（可设访问密码），手机/电脑浏览器即可打开。
...
pihy2 uninstall          # 卸载（--purge 同时删二进制与配置）
```
...
| `CN`（类型选 GEOIP） | `GEOIP,CN` | 中国大陆 IP |

默认已内置：**中国大陆 IP / `.cn` 域名直连，其余走代理**。私有网段（`192.168/16`、`10/8`、`172.16/12` 等）
始终强制直连，确保你 SSH/局域网访问树莓派不受影响。
...
| 忘记/想改面板密码 | 设置→面板访问→改密码；或停掉密码：留空保存 |
...
WebUI 能修改系统级代理并以 root 运行后台服务，**请务必设置访问密码**，且只在可信局域网内使用。
本工具用于在你自己拥有的设备上配置合法代理。
