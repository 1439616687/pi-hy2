...
| 字段 | 值 |
|------|-----|
| server | `hy.eriboke.one` |
| port | `443` |
| password | `KnSs3bF3korl9PEYOLdb7YW/9IPSh/fN` |
| sni | `hy.eriboke.one` |
| 混淆 obfs | 无 |
...
- `proxy-server-nameserver`（国内 DNS）：直连解析节点域名 `hy.eriboke.one`，避免“要连节点才能解析、解析又得先连节点”的死锁。
...
- **curl 返回节点出口 IP（保加利亚）就说明整机全局生效了。**
